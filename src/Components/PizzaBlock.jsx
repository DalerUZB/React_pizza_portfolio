...
const PizzaBlock = ({ imageUrl, name, sizes, price, types }) => {
    ...
    const pizzaActiveTypesFunc = (index) => {
        setActiveTypes(index)
    }

    const pizzaActiveSizesFunc = (index) => {
    ...
    return (
        <div className="pizza-block">
            ...
            <div className="pizza-block__selector">
                <ul>
                    {TypesItems.map((type, index) =>
                        <li key={type}
                            onClick={() => pizzaActiveTypesFunc(index)}
                ...
                <ul>
                    {aviableSizes.map((sizesItems, index) =>
                        <li
                            onClick={() => pizzaActiveSizesFunc(index)}
                            key={index}
                            className={ClassNames({
                                active: activeSizes === index,
                                disabled: !sizes.includes(sizesItems)
                            })}
